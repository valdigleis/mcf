...
\section{Teorema Myhill-Nerode e a  minimização de AFD}
...
\section{Máquinas de Mealy e Moore como Tradutores Finitos}
...
{\color{red} \scriptsize \textdbend} Para esse questionário sempre que $w \in \Sigma^*$ e $c \in \Sigma$ a notação $|w|_c$ irá representar o número de $c$'s que existem na palavra $w$.

\ 

\begin{questao}\label{exer:LR8}
...
\begin{figure}[H]
	...
	\end{tikzpicture}
	\caption{Autômato para o exercício \ref{exer:LR8}.}
...
\begin{questao}\label{exer:LR19}
	Dado os AFN que você construiu nos Exercícios \ref{exer:LR17} e \ref{exer:LR18}, encontre os AFD equivalentes a ele.
\end{questao}

\begin{questao}\label{exer:LR20}
...
\begin{figure}[H]
	...
	\end{tikzpicture}
	\caption{Autômato para o exercício \ref{exer:LR20}.}
...
\begin{figure}[H]
	...
	\end{tikzpicture}
	\caption{Autômato para o exercício \ref{exer:LR21}.}
...
\begin{table}[H]
	...
	\begin{tabular}{c|cccc}
		\diagbox{$Q'$}{$\Sigma$}	& $\lambda$ & $a$ & $b$ & $c$\\\hline
		$q_0$ & $\emptyset$ & $\{q_0\}$ & $\{q_1\}$ & $\{q_2\}$\\
		$q_1$ & $\{q_0\}$ & $\{q_1\}$ & $\{q_2\}$ & $\emptyset$\\
		$q_2$ & $\{q_1\}$ & $\{q_2\}$ & $\emptyset$ & $\{q_0\}$ 		
	\end{tabular}
	\caption{Tabela da função de transição do $\lambda$-AFN do exercício \ref{exer:LR25}.}
...
\begin{table}[H]
	\centering
	\begin{tabular}{c|cccc}
		\diagbox{$Q'$}{$\Sigma$}	& $\lambda$ & $0$ & $1$ & $2$\\\hline
		$s_0$ & $\{s_1, s_2\}$ & $\emptyset$  & $\{s_1\}$ & $\{s_2\}$\\
		$s_1$ & $\emptyset$ & $\{s_0\}$ & $\{s_2\}$ & $\{s_0, s_2\}$ \\
		$s_2$ & $\emptyset$ & $\emptyset$ & $\emptyset$ & $\emptyset$ 		
	\end{tabular}
	\caption{Tabela da função de transição do $\lambda$-AFN do exercício \ref{exer:LR26}.}
...
\begin{questao}\label{exer:LR28}
	Encontre o AFD quociente do AFD apresentado no exercício \ref{exer:LR8}.
\end{questao}

\begin{questao}\label{exer:LR29}
	Encontre o AFD quociente equivalente ao AFN apresentado no exercício \ref{exer:LR20}.
\end{questao}

\begin{questao}\label{exer:LR30}
	Encontre o AFD quociente equivalente ao AFN apresentado no exercício \ref{exer:LR21}.
\end{questao}

\begin{questao}\label{exer:LR31}
	Encontre o AFD quociente equivalente ao AFN apresentado no exercício \ref{exer:LR25}.
\end{questao}

\begin{questao}\label{exer:LR32}
	Encontre o AFD quociente equivalente ao AFN apresentado no exercício \ref{exer:LR26}.
\end{questao}

\begin{questao}\label{exer:LR33}
...
\begin{figure}[H]
	...
	\end{tikzpicture}
	\caption{Autômato para o exercício \ref{exer:LR33}.}
	\label{fig:AutomaExercicioLR33}
\end{figure}

\begin{questao}\label{exer:LR34}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[>=stealth, shorten >=1pt, node distance=2.5cm, on grid, auto, state/.append style={minimum size=3em}, thick ]
		...
		(C) edge [loop above]				node {$0$}				 ( );
	\end{tikzpicture}
	\caption{Autômato para o exercício \ref{exer:LR34}.}
	\label{fig:AutomaExercicioLR34}
\end{figure}

\begin{questao}\label{exer:MM35}
	Esboce uma máquina de Mealy que converte a representação monetária americana para a representação brasileira. Por exemplo, se o autômatos receber como entrada a palavra $US\$65,010.59$ ela deve devolver como saída a palavra $BR\$65.010,59$.
\end{questao} 

\begin{questao}\label{exer:LR36}
	Esboce uma máquina de Moore que implementa um chatbot de \textit{Telegram} para uma empresa que oferece suporte de informática.
\end{questao}

\begin{questao}\label{exer:LR37}
	Construa uma máquina de Mealy que, dado um alfabeto $\Sigma = \{0,1\}$, computa a seguinte função: para cada sub-sequência de dois $1$ seguidos, o segundo deve ser traduzido para $A$ se o antecessor não tiver sido traduzido, por exemplo,  uma palavra $010110111$ deve ser traduzida para $0101A01A1$.
\end{questao}

\begin{questao}\label{exer:LR38}
	Construa uma máquina de Mealy que, ao receber uma sequência de bits de entrada, inverte o bit atual se o número de bits $1$ já processados for par, e não faz nada se for ímpar.
\end{questao}

\begin{questao}\label{exer:LR39}
	Construa uma máquina de Moore que, ao receber uma sequência binária de entrada, em a máquina emite $1$ sempre que a sequência até aquele ponto lido na memória de entrada tiver um número ímpar de $1$'s, caso contrário a máquina deve emitir e $0$ se contiver um número par de $1$'s.
\end{questao}

\begin{questao}\label{exer:LR40}
	Apresente um algoritmo de minimização para máquinas de Mealy e Moore, e explique de forma sucinta a sutiliza na diferenca entre os dois algoritmos.
\end{questao}