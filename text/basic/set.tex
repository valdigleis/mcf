...
Além destes conjuntos básicos, alguns subconjuntos deles também merecem destaque, e por isso tem palavras reservadas para eles.
...
%\section{Conjuntos Numéricos e Intervalos}\label{sec:ConjuntoNumericoIntervalos}

%Um tipo específico de conjunto que é de sua importância para a matemática e áreas afim, são os intervalos numérico, tais conjuntos tem um representação própria dentro da teoria do conjuntos. Antes de formalizar o conceito de intervalo numérico, é interessante apresentar a noção de reta real. 

%A reta real como tido em \cite{lipschutz1978-TC} é uma representação do conjunto dos números reais na forma de uma reta onde estão dispostos os elementos de $\mathbb{R}$, o elemento $0$ encontra-se no centro da reta, sendo esta infinita à esquerda e à direita do $0$, ressaltando que o $0$ é chamado de origem da reta real. 
...
%	\begin{tikzpicture}
		% Reta
%		\draw[-] (-5,0) -- (5,0);
%		\draw[thick] (5.5,0.2) node[below] {$\cdots$};
%		\draw[thick] (-5.5,0.2) node[below] {$\cdots$};
		% Marcadores
%		\draw[thick] (0,0) node {$|$};
%		\draw[thick] (1,0) node {$|$};
%		\draw[thick] (1.41,0) node {$|$};
%		\draw[thick] (2,0) node {$|$};
%		\draw[thick] (2.5,0) node {$|$};
%		\draw[thick] (4.5,0) node {$|$};
%		\draw[thick] (-1,0) node {$|$};
%		\draw[thick] (-2.7,0) node {$|$};
%		\draw[thick] (-4,0) node {$|$};
%		\draw[thick] (-5,0) node {$|$};
		% Os números
%		\draw[thick] (0,-0.5) node {$0$};
%		\draw[thick] (1,-0.5) node {$1$};
%		\draw[thick] (1.41,-0.5) node {$\sqrt{2}$};
%		\draw[thick] (2,-0.5) node {$2$};
%		\draw[thick] (2.5,-0.5) node {$2.5$};
%		\draw[thick] (3.14,-0.5) node {$\pi$};
%		\draw[thick] (4.5,-0.5) node {$4.5$};
%		\draw[thick] (-1.1,-0.5) node {$-1$};
%		\draw[thick] (-2.8,-0.5) node {$-e$};
%		\draw[thick] (-4.1,-0.5) node {$-4$};
%		\draw[thick] (-5.1,-0.5) node {$-5$};
%	\end{tikzpicture}
%	\caption{Uma representação parcial da reta real, exibindo a localização de alguns números reais.}
%	\label{fig:RetaReal}
%\end{figure}
